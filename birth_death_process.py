...
# ---------------------------
# Example 1: initial condition with A(0) = 0
# ---------------------------
k1 = 0.1
k2 = 1.0
x0 = 0
...
for r in range(n_rep):
    ax.step(time_grid, traj_grid[r], where='post', lw=1, label=f'SSA trajectory {r+1}')
...
ax.grid(which='major', axis='both', linestyle=':', linewidth=0.6)
ax.set_xlabel('time [s]')
ax.set_ylabel('A molecules', )
ax.set_title('Five SSA runs of production-degradation system with A(0) = 0')
ax.legend()
plt.tight_layout()
plt.show()
...
# ---------------------------
# Example 2: initial condition with A(0) = 100
# ---------------------------
k1 = 0.1
k2 = 1.0
x0 = 100
...
# analytic mean M(t) = x0 * exp_term + lam * (1 - exp_term)
lam = k2 / k1
exp_term = np.exp(-k1 * time_grid)
analytic_mean = x0 * exp_term + lam * (1 - exp_term)
...
ax.set_xlabel('time [s]')
ax.set_ylabel('A molecules', )
ax.set_title('Five SSA runs of production-degradation system with A(0) = 100')
ax.legend()
plt.tight_layout()
plt.show()
